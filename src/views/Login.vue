    ...
    <div class="login-container">
      ...
      <form>
        ...
        <div class="form-button">
          <button v-on:click.prevent="handleLogin">Enter <ArrowRightIcon /></button>
        </div>
...
export default class Login extends Vue {
  ...
  async handleLogin(): Promise<void>{
    if(!this.email || !this.password){
      ...
      try {
        const res = await this.$api.post('/sessions', {
          email: this.email,
          password: this.password
        });

        localStorage.setItem('@todoToday:token', res.data.token);
